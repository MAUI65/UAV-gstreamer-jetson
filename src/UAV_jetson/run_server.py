...
async def async_main():
    ...
    parser.add_argument( '--udp', help='set UDP IP address, ie: 10.42.0.1')
    parser.add_argument( '--mav', help='set mav connection, ie: udpout:10.42.0.1:14445')   
    args = parser.parse_args()

    print(f"{args.udp = }, {args.mav = }")
    ...
    if args.udp:
        config_dict['camera_UDP_IP'] = args.udp
    if args.mav:
        config_dict['mavlink']['connection'] = args.mav

    mav_connection = config_dict['mavlink']['connection']
    udp_ip = config_dict['camera_UDP_IP']

    print(f"{mach = }, {conf_path = } {mav_connection = } {udp_ip = } \n")
    print(config_dict)
