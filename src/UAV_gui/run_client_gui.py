...
async def async_main(config_dict):
    try:
...
def main():
    ...
    parser.add_argument( '--mav', help='set mav connection, ie: udpin:10.42.0.1:14445')   
    args = parser.parse_args()

    config_dict = toml_load(config_dir() / f"client_config.toml")

    if args.connect_local:
        print("overwrite config_dict['connection'] with local connection")
        config_dict['mavlink']['connection'] = "udpin:localhost:14445"
